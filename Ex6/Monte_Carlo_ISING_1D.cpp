...
int main()
{
  ...
  for(int i=0;i<neq;i++) //equilibration
    Move(metro);
...
void Move(int metro)
{
  ...
  //Select randomly a particle (for C++ syntax, 0 <= o <= nspin-1)
    ...
    if(metro==1){ //Metropolis
      ...
      energy_new=Boltzmann((int) -s[o],o);
      ...
      prob_up=1./(1.+exp(-beta*(energy_down-energy_up)));
      if((prob_up-move)<0.){
        s[o]=-1;
      }else{
        s[o]=1;
...
//cycle over spins
  ...
  //measure needed for Internal Energy
  walker[iu] = u;
  //measure needed for Heat Capacity
  walker[ic] = u*u;
  //measure needed for Magnetization
  walker[im] = m;
  //measure needed for Susceptibility
  walker[ix] = m*m;
}
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    stima_c = beta*beta*((blk_av[ic]/blk_norm)-pow(stima_u,2)); //Heat
    glob_av[ic]  += stima_c;
    ...
    stima_m = blk_av[im]/blk_norm; //Mag
    glob_av[im]  += stima_m;
    ...
    stima_x = beta*blk_av[ix]/blk_norm; //Chi
    glob_av[ix]  += stima_x;
    ...
    //printing result at the end of simulation
    if(iblk==nblk){

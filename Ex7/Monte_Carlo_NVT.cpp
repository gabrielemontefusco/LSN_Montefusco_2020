...
//Read input informations
  ...
  ReadInput.open("bool.dat"); //The program has already runned?
  ReadInput >> prv_sim;
  ReadInput.close();
...
// contribution to energy and virial
  ...
  WriteConf.open("InstaVal.dat",ios::app);
  WriteConf<<walker[iv]/(double)npart + vtail<<"\t"<<walker[iw] + rho * temp + ptail * (double)npart / vol <<endl;
...
//g(r)
    for (int i=0; i<nbins; i++){
      r=i*bin_size;
      vol_g=4*pi/3*(pow(r+bin_size,3)-pow(r,3));
      gdir=blk_av[igofr+i]/(blk_norm*rho*npart*vol_g);

...
double variationalWaveFunction_second(double v)
{
	return 0;
	/*double appom,appop;
    appom=exp(-pow((v-mu),2)/(2*sigma*sigma));
    appop=exp(-pow((v+mu),2)/(2*sigma*sigma));
    return -appom/(sigma*sigma)-appop/(sigma*sigma)+pow(v-mu,2)*appom/pow(sigma,4)+pow(v+mu,2)*appop/pow(sigma,4);*/

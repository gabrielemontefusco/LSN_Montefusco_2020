...
double e_tot(double x,double mu,double sigma){
  double psi,e_kin,e_pot,appop,appom;

  appom=exp(-pow((x-mu),2)/(2*sigma*sigma));
  appop=exp(-pow((x+mu),2)/(2*sigma*sigma));
  psi=appom+appop;
  e_kin=-0.5*(-appom/(sigma*sigma)-appop/(sigma*sigma)+pow(x-mu,2)*appom/pow(sigma,4)+pow(x+mu,2)*appop/pow(sigma,4));
  e_pot=pow(x,4)-2.5*pow(x,2);

  return (e_kin/psi+e_pot);
